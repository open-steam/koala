...
function revealPath($savedPath,$currentPath=""){
	...
	if(strtolower(substr($savedPath,0,4))=="http"){return $savedPath;}
	if(strtolower(substr($savedPath,0,3))=="www"){return "http://".$savedPath;}
	if(strtolower(substr($savedPath,0,7))=="mailto:"){return $savedPath;}
...
function detectMimeType($pathString){
	...
	while(($line = fgets($handle, 4096)) !== false){
		...
		if (!$existing) fwrite($handleWrite, $line . "\n");
	}
