...
class lms_wiki extends koala_object
{
	private $template;
	private $steam_wiki;
	...
	public function set_admin_menu( $context, $wiki_obj )
	{
		...
		if ($context == "index") {
			if ($wiki_obj->check_access_insert( $user ) && !$wiki_obj->check_access_write( $user )) {
				$index_menu[] = array("link" => PATH_URL . "wiki/postentry/" . OBJ_ID  , "name" => gettext( "Create new entry" ));
			}
			if (WIKI_MEDIATHEK && $wiki_obj->check_access_write($user)) $index_menu[] = array("link" => PATH_URL . "wiki/mediathek/" . OBJ_ID  , "name" => gettext( "Mediathek" ));

			if ($wiki_obj->check_access_write( $user )) {
				$index_menu[] = array("link" => PATH_URL . "wiki/postentry/" . OBJ_ID , "name" => gettext( "Create new entry" ));
				(WIKI_EDIT) ? $index_menu[] = array("link" => PATH_URL . "wiki/editwiki/" . OBJ_ID , "name" => gettext( "Preferences" )):"";
				if ($wiki_obj->check_access_move( $user)) {
					(WIKI_DELETE) ? $index_menu[] = array("link" => PATH_URL . "wiki/deletewiki/" . OBJ_ID  , "name" => gettext( "Delete" )):"";
				}
				$entry_menu[] = array("link" => PATH_URL . "wiki/editentry/" . OBJ_ID , "name" => gettext( "Edit entry" ));
			}
		}
		...
		if ($context == "entry") {
			if ($wiki_obj->check_access_write( $user )) {
				$index_menu[] = array("link" => PATH_URL . "wiki/postentry/" . OBJ_ID , "name" => gettext( "Create new entry" ));
				$index_menu[] = array("link" => PATH_URL . "wiki/editentry/" . OBJ_ID , "name" => gettext( "Edit Description" ));
				if ($wiki_obj->check_access_move( $user)) {
					$index_menu[] = array("link" => PATH_URL . "wiki/deleteentry/" . OBJ_ID , "name" => gettext( "Delete" ));
				}
				
				$startpage = $wiki_obj->get_environment()->get_attribute("OBJ_WIKI_STARTPAGE");
				...
				$entry_menu[] = array("link" => PATH_URL . "wiki/editentry/" . OBJ_ID , "name" => gettext( "Edit entry" ));
				if ($wiki_obj->check_access_move( $user)) {
					$entry_menu[] = array("link" => PATH_URL . "wiki/deleteentry/" . OBJ_ID , "name" => gettext( "Delete" ));
				}
			}
		}
		...
		{
			if ($wiki_obj->check_access_write( $user )) {
				$mediathek_menu[] = array( "link" => PATH_URL . "wiki/upload/?env=". $wiki_obj->get_id() . "/", "name" => gettext("Upload new image"));
			}
		}

		if ($context == "version")
		{
			$wiki_orig = steam_factory::get_object( $GLOBALS["STEAM"]->get_id(), OBJ_ID);
			if( $wiki_orig->check_access_write( $user))
				$version_menu[] = array( "link" => PATH_URL . "wiki/recoverversion/" . OBJ_ID . "/" . $wiki_obj->get_id() , "name" => gettext("Recover version"));
		}
	...
	if( $no_versions > 0)
	{
		...
		for( $i = $from; $i < $to; $i++ )
		{
			...
			{
				$t->setVariable( "WIKI_DOC_LINK", PATH_URL . "wiki/viewversion/" . $wiki_doc->get_id() . "/" . $prev_versions[$i]->get_id() . "/" );
				$t->setVariable( "WIKI_DOC_VERSION", "Version " . h($attributes_result[$attributes_tnr[$prev_versions[$i]->get_id()]][DOC_VERSION]) );
				$t->setVariable( "AUTHOR_LINK", PATH_URL . "user/index/" . $author_result[$author_tnr[$prev_versions[$i]->get_id()]][OBJ_NAME] . "/" );
