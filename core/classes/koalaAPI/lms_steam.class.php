...
class lms_steam
{
	 ...
	 */
	static public function get_link_path( $obj, $offset = FALSE, $length = FALSE ) {
		$path_objects = array();
		$parent = $obj;
	...
	static public function get_link_path_html( $obj, $offset = FALSE, $length = FALSE )
	{
		if ( is_object( $obj ) )
...
// strange:
	public function get_link_to_root( $obj )
	{
		...
		if ( sscanf( $p[ 1 ], "~%s", $username ) != FALSE && isset( $username ) && is_string( $username ) && is_object( $user = steam_factory::username_to_object( $GLOBALS[ "STEAM" ]->get_id(), $username ) ) ) {
          ...
          } else if ( is_object( $user = steam_factory::username_to_object( $GLOBALS[ "STEAM" ]->get_id(), $p[ 2 ] ) ) ) {
            // Target is a user
