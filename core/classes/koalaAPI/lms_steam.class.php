...
class lms_steam {
    ...
    public static function connect($server = STEAM_SERVER, $port = STEAM_PORT, $login = STEAM_GUEST_LOGIN, $password = STEAM_GUEST_PW) {
        try {
            ...
            throw new Exception(
            "No connection to sTeam server ($server:$port).", E_CONNECTION
            );
        }
    }
        ...
        foreach ($path_objects as $path_obj) {
            switch (TRUE) {
                ...
                case $path_obj instanceof steam_group:
                    ...
                    if (!isset($courses_group))
                        $courses_group = steam_factory::get_object($GLOBALS["STEAM"]->get_id(), STEAM_COURSES_GROUP, CLASS_GROUP);
...
// strange:
    ...
    public function count_user() {
        ...
        if (is_object($all_user))
            return $all_user->count_members();
        else
            return -1;
    }
            ...
            switch ($type) {
                case( "login" ):
                    $result_login = $GLOBALS["STEAM"]->predefined_command(
                            $user_module, "lookup_login", array($part, $like), 0
                    );
                    ...
                    $result_email = $GLOBALS["STEAM"]->predefined_command(
                            $user_module, "lookup_email", array($part, $like), 0
                    );
                ...
                default: // NAME
                    ...
                    if (sizeof($names) >= 2) {
                        ...
                        $result = $GLOBALS["STEAM"]->predefined_command(
                                $user_module, "lookup_name", array($firstname, $lastname, $like), 0
                        );
                    } else {
                        $result = $GLOBALS["STEAM"]->predefined_command(
                                $user_module, "search_name", array($part, $part, $like), 0
                        );
                    }
                    if (is_object($result))
                        $result = array($result);
...
// Seems to be never used
    ...
    public function user_get_contacts_to_confirm($username) {
        ...
        if (!is_array($contacts_toconfirm) || count($contacts_toconfirm) == 0)
            return array();
        $attributes = array(
            USER_FIRSTNAME,
            USER_FULLNAME,
        ...
        foreach ($vusers as $user) {
            ...
            if (is_object($user->get_attribute("USER_PROFILE_FACULTY"))) {
                ...
                if (empty($facname) || $facname = 0 || strlen($facname) == 0)
                    $facname = gettext("miscellaneous");
            } else
                $facname = gettext("miscellaneous");
...
// user_get_unconfirmed_contacts ssems to be never used
    ...
    public function get_annotations($doc_id) {
        ...
        foreach ($annotations as $annotation) {
            $more_tnr[$i] = array();
            if (strstr($data_result[$data_tnr[$i]["ATTRIBUTES"]][DOC_MIME_TYPE], "text"))
                $more_tnr[$i]["CONTENT"] = $annotation->get_content(TRUE);
            $result["OBJ_ID"] = $annotation->get_id();
        ...
        foreach ($annotations as $annotation) {
            ...
            if (strstr($result["DOC_MIME_TYPE"], "text"))
                $result["CONTENT"] = $more_result[$more_tnr[$i]["CONTENT"]];
            $result["OBJ_ID"] = $annotation->get_id();
...
// Seems to be never used
    ...
    public static function is_reader($steam_doc, $steam_user, $pBuffer = FALSE) {
        ...
        return $GLOBALS["STEAM"]->predefined_command(
                        $read_mod, "is_reader", array($steam_doc, $steam_user), $pBuffer
        );
    }
    ...
    public function search_user_posts($message_board, $user) {
        ...
        return $GLOBALS["STEAM"]->predefined_command(
                        $search_mod, "search_user_posts", array($message_board->get_id(), $user), 0
        );
    }
    ...
    public function search_messageboard($message_board, $pattern) {
        ...
        return $GLOBALS["STEAM"]->predefined_command(
                        $search_mod, "search_messageboard", array($message_board->get_id(), $pattern), 0
        );
    }
    ...
    public function get_readers($steam_doc, $pBuffer = FALSE) {
        ...
        return $GLOBALS["STEAM"]->predefined_command(
                        $read_mod, "get_readers", array($steam_doc), $pBuffer
        );
    }
...
//error_log("lms_steam::mail(): sending email. subject=" . $pSubject . " messagebody=" . $HtmlMessageBody . " senderstring=" . $senderstring);
    ...
    public function delete($obj = FALSE) {
        $trashbin = \lms_steam::get_current_user()->get_trashbin();
        if (!is_object($obj))
