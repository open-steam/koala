...
class DeleteEntry extends \AbstractCommand implements \IFrameCommand, \IIdCommand, \IAjaxCommand {
	...
	public function processData(\IRequestObject $requestObject){
		...
		if(count($newEntries) == 0){
			\ExtensionMaster::getInstance()->callCommand("DeleteCategory", "PortletTopic", array('portletId'=>$objectId,'categoryIndex'=>$this->categoryIndex));
		}
		else{
			$content[$this->categoryIndex]["topics"]=$newEntries;
