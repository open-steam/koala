...
class Index extends \AbstractCommand implements \IFrameCommand {
	...
	public function frameResponse(\FrameResponseObject $frameResponseObject) {
		...
		if ($admin == 1 && $surveyCount == 0) {
			header('Location: ' . $QuestionnaireExtension->getExtensionUrl() . "edit/" . $this->id . "/");
    	die();
		}
		...
		if (count($surveys) == 0) {
			...
			$content->setVariable("QUESTIONNAIRE_NAME", $questionnaire->get_name());
			if ($questionnaire->get_attribute("QUESTIONNAIRE_DESC") != "0") {
				$content->setVariable("QUESTIONNAIRE_DESC", nl2br($questionnaire->get_attribute("QUESTIONNAIRE_DESC")));
			}
