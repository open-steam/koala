...
class ManageGroup extends \AbstractCommand implements \IFrameCommand {
	
	private $params;
	
	public function validateData(\IRequestObject $requestObject) {
		$this->params = $requestObject->getParams();
		// TODO: Diese Methode anpassen
		return true;
			      ...
			      $problems .= gettext("Please don't use the \".\"-char in the groupname.") . ' ';
					...
					if ( empty( $problems ) ) {
				          ...
				          if ($is_public)  
				        	...
				        	if (is_object($parent)) 
				        		$pgroup_id = $parent->get_id(); // a koala group
				          	else 
				          		$pgroup_id = -1; // no koala group. its a steam only group
				        }
