...
class Explorer extends AbstractExtension implements IIconBarExtension {
    ...
    public function getCurrentObject(UrlRequestObject $urlRequestObject) {
        ...
        if (isset($id)) {
            ...
            $currentUser = $GLOBALS["STEAM"]->get_current_steam_user();
            if(!$currentUser instanceof \steam_user) throw new Exception ("The current user cannot be detemrined. The variable is of the type ".gettype($currentUser).". The class is ".get_class($currentUser));
