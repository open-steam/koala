  ...
  $.fn.pageSlide = function(options) {
    ...
    var settings = $.extend({
      ...
      if ($("#pageslide-body-wrap, #pageslide-content, #pageslide-slide-wrap").size() == 0) {
        ...
        $("#content_wrapper").contents().wrapAll( psBodyWrap );
      ...
      $(anchor).attr("rel","pageslide");
		...
		html = document.getElementById(settings.content).innerHTML;
    	$("#pageslide-slide-wrap").animate({width: settings.width}, settings.duration);
	      ...
	      //$.ajax({
  		  ...
  		  //    success: function(data){
  		        //$("#pageslide-content").css("width",settings.width).html(html + "<br><a href='javascript:;' class='pageslide-close'>schlie&szlig;en</a>")
  		        $("#pageslide-content").css("width",settings.width).html(html)
  		          .queue(function(){
  		            $(this).dequeue();
  				...
  				if (jQuery.isFunction(settings.handler)) {
  					settings.handler();
  				}
  		           	$("#pageslide-content a").click(function(elm){
						elm.stopPropagation();
			    		$.fn.pageSlideClose();
					});
	    ...
	    if(settings.modal == true) {
	      ...
	      $("#pageslide-blanket").click(function(elm){
  		    _closeSlide(elm);
	...
	$("#pageslide-content").click(function(elm){
    	$.fn.pageSlideClose();
    	if(settings.modal == true && $("#pageslide-blanket").is(":visible")) {
      		$("#pageslide-blanket").animate({opacity:'0.0'}, 'fast','linear',function(){$(this).hide();});
    	}
...
(function($){
  $(document).ready(function(){
    $(document).keyup(function(event){
      ...
      if (event.keyCode == 27) {
      	$.fn.pageSlideClose();
      	if($("#pageslide-blanket").is(":visible")) {
      		$("#pageslide-blanket").animate({opacity:'0.0'}, 'fast','linear',function(){$(this).hide();});
    	}
      }
    });
