<?php
if (!defined("PATH_TEMPLATES_UNITS_VIDEOSTREAMING")) define( "PATH_TEMPLATES_UNITS_VIDEOSTREAMING", PATH_EXTENSIONS . "units_videostreaming/templates/" );
	...
	if ( empty( $problems ) )
	{
    	...
    	if (! isset($unit) ) 
    	{
			...
			$new_unit_vilm->set_attributes(array(
    								"UNIT_TYPE" => "units_videostreaming",
    								"OBJ_TYPE" => "videostreaming_unit_koala",
									"UNIT_DISPLAY_TYPE" => gettext("units_videostreaming")
			));
...
else
{
	...
	$content->loadTemplateFile( PATH_TEMPLATES_UNITS_VIDEOSTREAMING . "units_videostreaming_new.template.html" );
	...
	$content->setVariable( "CONFIRMATION_TEXT_LONG", gettext( "You are going to add a new videostreaming unit to this course." ) );
	$content->setVariable( "LABEL_NAME", gettext( "Name" ) );
	
	
	$content->setVariable( "UNIT", "units_videostreaming" );
