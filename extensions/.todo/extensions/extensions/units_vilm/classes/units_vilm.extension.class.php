<?php
require_once( PATH_EXTENSIONS . "units_vilm/classes/koala_object_vilm.class.php");

class units_vilm extends koala_unit
{
	...
	function handle_path( $path, $owner = FALSE, $portal = FALSE ) {
		...
		if ( isset( $path[0] ) && is_numeric( $path[0] ) ) {
			...
			if ( is_object( $steam_unit ) && $steam_unit->get_attribute( "UNIT_TYPE" ) !== "units_vilm" )
				return;
		...
		if (count($path) > 1 && $path[1] == "config") {
			...
			$action="config";
			include( PATH_EXTENSIONS . 'units_vilm/modules/units_vilm.php' );
		} else {
			$action = "index";
			include( PATH_EXTENSIONS . 'units_vilm/modules/units_vilm.php' );
		}
	...
	{
		if ( $obj_type === "vilm_unit_koala" ) {
			return new koala_object_vilm( $steam_object, new units_vilm( lms_steam::get_root_creator( $steam_object ) ) );
		}
		return FALSE;
	}
