...
function insertElement(name)
{
	if(name!="")
	{
		...
		input.setAttribute("name", "grading_option_"+id);
		input.setAttribute("id", "grading_option_"+id);
...
function getnewID()
{
	...
	while(found==false)
	{
		...
		if(document.getElementById("grading_option_"+randomID))	found=false;	
		else found=true;
	}
	return randomID;
}
...
{	
	...
	{	
		...
		document.getElementById("grading_option_"+optionid).value=content;
