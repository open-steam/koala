<?php
  ...
  ****************************************************************************/
  ...
  $current_room_data = $current_room->get_attributes(array(OBJ_NAME, OBJ_DESC, OBJ_TYPE, OBJ_LAST_CHANGED, "bid:presentation", "bid:collectiontype"), 1);
  $current_room_creator = $current_room->get_creator(1);

  //check if user may write in this folder
  $login_user = $steam->get_login_user();
  $write_allowed = $current_room->check_access_write($login_user, 1);
  //get inventory and inventorys attributes if allowed to
  $allowed = $current_room->check_access_read($login_user, 1);
  ...
  if($allowed && $current_room instanceof steam_container)
    $inventory = $current_room->get_inventory("", array(OBJ_TYPE, "DOC_MIME_TYPE", "DOC_LAST_MODIFIED", "CONT_LAST_MODIFIED", "OBJ_LAST_CHANGED", "bid:presentation", "bid:collectiontype", "bid:hidden", "bid:doctype", "DOC_EXTERN_URL", "OBJ_CREATION_TIME"));
  else
    $inventory = array();
  ...
  $tpl->set_file("content", "contentframe.ihtml");
  ...
  $tpl->set_var(array(
    ...
    "FOLDER_NAME_STRIPPED" => strip_tags($current_room_display_name),
  ...
  if ($write_allowed && $login_user->get_name() != "guest") {
  ...
  foreach($inventory as $item) {
    ...
    $tnr[$item->get_id()]["writeaccess"] = $item->check_access_write($login_user, 1);
   ...
   //parse correct Properties symbol
	...
	"obj_type" => $item->get_attribute("OBJ_TYPE")
      ...
      if($itemmimetype === "text/html" || $itemmimetype === "text/plain"
         ...
         || $itemmimetype === "application/vnd.google-earth.kml+xml" || $itemmimetype === "application/vnd.google-earth.kmz"
