...
function sendMultiRequest(command, paramsArray, elementIdArray, requestType, completeFunction, successFunction, namespace, message, loop, count) {
	if (paramsArray.length >= 1) {
		...
		sendRequest(command, params, elementId, requestType, function(){sendMultiRequest(command, paramsArray, elementIdArray, requestType, completeFunction, successFunction, namespace, message, loop + 1, count);}, successFunction, namespace);
	} else {
		closeDialog();
	}
}
