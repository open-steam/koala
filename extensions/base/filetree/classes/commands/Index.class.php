...
class Index extends \AbstractCommand implements \IAjaxCommand {
    ...
    private function getOneRootHTML($currentUser, $containerObject, $room, $root) {
        ...
        $html .= "<li class=\"directory " . $css . "\"><a href=\"" . $url . "\" rel=\"" . $room->get_id() . "\" class=\"" . $cssHighlight . "\"><svg class='svg'><use xlink:href='" . PATH_URL . "explorer/asset/icons/mimetype/svg/folder.svg#folder'/></svg> " . getCleanName($room, -1) . "<span style='color: #AAAAAA;'>" . $desc . "</span></a>";
        ...
        if ($containerObject->check_access_read() && $containerObject instanceof \steam_container) {
            foreach ($containerObject->get_inventory_filtered(array(array('+', 'class', CLASS_ROOM))) as $object) {
                if (getObjectType($object) === "room" && !$this->isHiddenItem($object, $currentUser) && !($containerObject->get_id() === $bid->get_id() && ($object->get_name() === "home" || $object->get_name() === "scripts"))) {
                    ...
                    $html .= "<li class=\"directory " . $css . "\"><a href=\"" . $url . "\" rel=\"" . $object->get_id() . "\" class=\"" . $cssHighlight . "\"><svg class='svg'><use xlink:href='" . PATH_URL . "explorer/asset/icons/mimetype/svg/folder.svg#folder'/></svg> " . getCleanName($object, -1) . "<span style='color: #AAAAAA;'>" . $desc . "</span></a>";
