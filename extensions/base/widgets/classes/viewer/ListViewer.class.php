...
class ListViewer extends Widget {
    ...
    public function getHtml() {
        ...
        foreach ($this->headlineProvider->getHeadlines() as $key => $headline) {
            ...
            if (method_exists($this->headlineProvider, "getOnClickHandler"))
                $this->getContent()->setVariable("LISTVIEWER_ITEM_HEAD_ONCLICK", $this->headlineProvider->getOnClickHandler($headline));
            if (method_exists($this->headlineProvider, "getOnMouseOverHandler"))
                $this->getContent()->setVariable("LISTVIEWER_ITEM_HEAD_ONMOUSEOVER", $this->headlineProvider->getOnMouseOverHandler($headline));
            if (method_exists($this->headlineProvider, "getOnMouseOutHandler"))
                $this->getContent()->setVariable("LISTVIEWER_ITEM_HEAD_ONMOUSEOUT", $this->headlineProvider->getOnMouseOutHandler($headline));
            $widths = $this->headlineProvider->getHeadLineAbsoluteWidths();
            if ($widths[$key] != "0") {
                $this->getContent()->setVariable("LISTVIEWER_HEAD_ITEM_WIDTH", "width:" . $widths[$key] . "px;");
            }
            $classes = $this->headlineProvider->getHeadLineClasses();
            if ($classes[$key] != "") {
                $this->getContent()->setVariable("LISTVIEWER_HEAD_ITEM_CLASS", $classes[$key]);
        ...
        if (count($this->content) == 0) {
            ...
            foreach ($this->content as $contentItem) {
                if ($this->isHiddenItem($contentItem, $itemCount)) {
                    $itemCount++;
                    continue;
                }
