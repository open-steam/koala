...
///auth_mod->suspend_user( user, !is_suspended );

$users_module = $GLOBALS["STEAM"]->get_module("users");

$auth_module = $GLOBALS["STEAM"]->get_module("auth");

$users = $GLOBALS["STEAM"]->predefined_command($users_module, "get_users", array(), false);
...
foreach ($users as $u) {
	$count++;
	$timestamp = $u->get_attribute(USER_LAST_LOGIN);
	if ($timestamp != 0) {
		$last_login = date('d.n.Y', $timestamp)." um ".date('H:i', $timestamp);
	} else {
		$last_login = "never";
	}
	
	$is_suspended = $GLOBALS["STEAM"]->predefined_command($auth_module, "is_user_suspended", array($u), false);
	
	$html .= "<tr><td>".$u->get_id()."</td><td>".$u->get_name()."</td><td>".$u->get_full_name()."</td><td>".$last_login."</td><td>$is_suspended</td></tr>";
}
